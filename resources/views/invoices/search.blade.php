...
<form action="{{ route('invoice_search') }}" method="GET">
    <label for="">Choose Customer</label>
    <select name="customer_id" class="form-control form-select">
        <option value="">select customer</option>
        @foreach ($customers as $customer)
        <option value="{{ $customer->id }}" {{$request &&  $request->customer_id == $customer->id ? "selected" : ""  }}>{{ $customer->name }}</option>
        @endforeach
    </select><br>

    <label for="">Invoice number</label>
    <input class="form-control" type="text" name="invoice_number" value="{{ $request && $request->invoice_number ? $request->invoice_number : '' }}"> <br>

    <label for="">Invoice amount</label>
    <input class="form-control" type="number" name="invoice_amount" value="{{ $request && $request->invoice_amount ? $request->invoice_amount : '' }}"> <br>

    <label for="">Invoice date</label>
    <input class="form-control" type="date" name="invoice_date" value="{{ $request && $request->invoice_date ? $request->invoice_date : '' }}"> <br>
